...
import { Weekday } from './../weekday'; // Import Weekday enum
...
export class JtableComponent implements OnInit {
  ...
  saveAll(): void {
    ...
    // HTTP PUT request to save the table data to the backend endpoint
    this.http.put(this.apiUrl, this.data).subscribe(
      response => {
